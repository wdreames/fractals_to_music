...
def get_song_data(music_notes, duration=0.5):
    note_freqs = get_piano_notes()
    song = [get_wave(note_freqs[note], duration=duration) for note in music_notes.split('-')]
    song = np.concatenate(song)
    return song.astype(np.int16)


def get_chord_data(chords, duration=0.5):
    chords = chords.split('-')
    note_freqs = get_piano_notes()

    chord_data = []
    for chord in chords:
        data = sum([get_wave(note_freqs[note], duration=duration) for note in list(chord)])
